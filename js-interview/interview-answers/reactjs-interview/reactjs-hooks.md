...
### 🟦 Case 1: Props (state passed from parent)

- If the parent passes down the same value (reference-stable), useMemo can prevent recomputation.
- But if the parent creates a new object/array/function on each render, useMemo in the child won’t help unless
- You also wrap those values in useMemo/useCallback in the parent.

### 🟦 Case 2: Internal state
